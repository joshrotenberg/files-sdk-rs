...
pub struct FileMigrationLogEntity {
    #[serde(flatten)]
    pub data: serde_json::Map<String, serde_json::Value>,
}

#[derive(Debug, Clone)]
pub struct FileMigrationLogHandler {
    client: FilesClient,
}

impl FileMigrationLogHandler {
    ...
    pub async fn list(
        ...
        per_page: Option<i64>,
    ) -> Result<(Vec<FileMigrationLogEntity>, PaginationInfo)> {
        let mut endpoint = "/file_migration_logs".to_string();
        ...
        let logs: Vec<FileMigrationLogEntity> = response.json().await?;
        Ok((logs, pagination))
    }
}
...
mod tests {
    ...
    fn test_handler_creation() {
        ...
        let _handler = FileMigrationLogHandler::new(client);
    }
}
