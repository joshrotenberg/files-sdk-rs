...
pub struct SettingsChangeEntity {
    #[serde(flatten)]
    pub data: serde_json::Map<String, serde_json::Value>,
}

#[derive(Debug, Clone)]
pub struct SettingsChangeHandler {
    client: FilesClient,
}

impl SettingsChangeHandler {
    ...
    pub async fn list(
        ...
        per_page: Option<i64>,
    ) -> Result<(Vec<SettingsChangeEntity>, PaginationInfo)> {
        let mut endpoint = "/settings_changes".to_string();
        ...
        let logs: Vec<SettingsChangeEntity> = response.json().await?;
        Ok((logs, pagination))
    }
}
...
mod tests {
    ...
    fn test_handler_creation() {
        ...
        let _handler = SettingsChangeHandler::new(client);
    }
}
