...
pub struct FtpActionLogEntity {
    #[serde(flatten)]
    pub data: serde_json::Map<String, serde_json::Value>,
}

#[derive(Debug, Clone)]
pub struct FtpActionLogHandler {
    client: FilesClient,
}

impl FtpActionLogHandler {
    ...
    pub async fn list(
        ...
        per_page: Option<i64>,
    ) -> Result<(Vec<FtpActionLogEntity>, PaginationInfo)> {
        let mut endpoint = "/ftp_action_logs".to_string();
        ...
        let logs: Vec<FtpActionLogEntity> = response.json().await?;
        Ok((logs, pagination))
    }
}
...
mod tests {
    ...
    fn test_handler_creation() {
        ...
        let _handler = FtpActionLogHandler::new(client);
    }
}
