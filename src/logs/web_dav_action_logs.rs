//! WebDAV action logging

use crate::{FilesClient, PaginationInfo, Result};
use serde::{Deserialize, Serialize};

#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct WebDavActionLogEntity {
    #[serde(flatten)]
    pub data: serde_json::Map<String, serde_json::Value>,
}

#[derive(Debug, Clone)]
pub struct WebDavActionLogHandler {
    client: FilesClient,
}

impl WebDavActionLogHandler {
    ...
    pub async fn list(
        ...
        per_page: Option<i64>,
    ) -> Result<(Vec<WebDavActionLogEntity>, PaginationInfo)> {
        let mut endpoint = "/web_dav_action_logs".to_string();
        ...
        let logs: Vec<WebDavActionLogEntity> = response.json().await?;
        Ok((logs, pagination))
    }
}
...
mod tests {
    ...
    fn test_handler_creation() {
        ...
        let _handler = WebDavActionLogHandler::new(client);
    }
}
