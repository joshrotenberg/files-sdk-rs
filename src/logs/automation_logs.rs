...
pub struct AutomationLogEntity {
    #[serde(flatten)]
    pub data: serde_json::Map<String, serde_json::Value>,
}

#[derive(Debug, Clone)]
pub struct AutomationLogHandler {
    client: FilesClient,
}

impl AutomationLogHandler {
    ...
    pub async fn list(
        ...
        per_page: Option<i64>,
    ) -> Result<(Vec<AutomationLogEntity>, PaginationInfo)> {
        let mut endpoint = "/automation_logs".to_string();
        ...
        let logs: Vec<AutomationLogEntity> = response.json().await?;
        Ok((logs, pagination))
    }
}
...
mod tests {
    ...
    fn test_handler_creation() {
        ...
        let _handler = AutomationLogHandler::new(client);
    }
}
