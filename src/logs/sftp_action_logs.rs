...
pub struct SftpActionLogEntity {
    #[serde(flatten)]
    pub data: serde_json::Map<String, serde_json::Value>,
}

#[derive(Debug, Clone)]
pub struct SftpActionLogHandler {
    client: FilesClient,
}
...
impl SftpActionLogHandler {
    ...
    pub async fn list(
        ...
        per_page: Option<i64>,
    ) -> Result<(Vec<SftpActionLogEntity>, PaginationInfo)> {
        let mut endpoint = "/sftp_action_logs".to_string();
        ...
        let logs: Vec<SftpActionLogEntity> = response.json().await?;
        Ok((logs, pagination))
    }
}
...
mod tests {
    ...
    fn test_handler_creation() {
        ...
        let _handler = SftpActionLogHandler::new(client);
    }
}
