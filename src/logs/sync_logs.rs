...
impl SyncLogHandler {
    ...
    pub async fn list(
        ...
        per_page: Option<i64>,
    ) -> Result<(Vec<SyncLogEntity>, PaginationInfo)> {
        let mut endpoint = "/sync_logs".to_string();
        ...
        let logs: Vec<SyncLogEntity> = response.json().await?;
        Ok((logs, pagination))
    }
}
...
mod tests {
    ...
    fn test_handler_creation() {
        ...
        let _handler = SyncLogHandler::new(client);
    }
}
